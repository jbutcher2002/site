 ...
 *  Custom Menu Walker.
 */
class rooten_menu_walker extends Walker_Nav_Menu {
    var $has_child = false;
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        // create item output
        $id = apply_filters('rooten_nav_menu_item_id', '', $item, $args);
        ...
        // create link output
        $item_output = sprintf($format, $args->before, $attributes, $icon.$args->link_before.apply_filters('rooten_the_title', $item->title, $item->ID).$args->link_after, $args->after);

        $output .= apply_filters('rooten_walker_nav_menu_start_el', $item_output, $item, $depth, $args);
    }
